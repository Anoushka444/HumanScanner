...
def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text , clf): # clf is defined for custom classifier
    ...
    for (x,y,w,h) in features:
        ...
        id_user,_=clf.predict(gray_img[y:y+h,x:x+w])
        if id_user==1:
            cv2.putText(img,"Anoushka", (x,y-4), cv2.FONT_HERSHEY_COMPLEX , 0.6 , color , 1 , cv2.LINE_AA)
        #if id_user==2:
         #   cv2.putText(img,"Cheenu", (x,y-4), cv2.FONT_HERSHEY_COMPLEX , 0.6 , color , 1 , cv2.LINE_AA)
        if id_user==2:
            cv2.putText(img,"Pakhi Jadon", (x,y-4), cv2.FONT_HERSHEY_COMPLEX , 0.6 , color , 1 , cv2.LINE_AA)
...
def recog(img,clf,faceCascade):
    ...
    coordinate= draw_boundary(img,faceCascade, 1.1, 10 , color['white'],"face",clf)
    return img

def detect(img, faceCascade,eyesCascade, noseCascade,img_id ):
    color={"blue":(255,0,0) , "red":(0,0,255), "green":(0,255,0) ,"white":(255,255,255)}
    coordinate  = draw_boundary(img,faceCascade, 1.1, 10 , color['White'],"face" )

    #detectMultiScale(list) has coordinates for the face
    ...
    if len(coordinate)==4: #if coordinate isnt equaal to 4 face isnt detected (4 determines x,y,w,h)
        ...
        #user_id= 1 #id for a unique person
