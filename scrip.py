...
#def gen_dataset(img,id_user, img_id):
...
def detect(img, faceCascade,eyesCascade, noseCascade ):
    color={"blue":(255,0,0) , "red":(0,0,255), "green":(0,255,0) ,"white":(255,255,255)}
    coordinate  = draw_boundary(img,faceCascade, 1.1, 10 , color['blue'],"face")

    #detectMultiScale has coordinates for the face

    if len(coordinate)==4: #if coordinate isnt equal to 4 face isnt detected (4 determines x,y,w,h)
        ...
        coordinate = draw_boundary(roi_img, mouthCascade, 1.1, 25, color['white'], "mouth")
...
#earCascade= cv2.CascadeClassifier("Ear.xml")
